...
/* THE FIT CHECK, embed variant — five chip-taps, one honest verdict, inside
   the LP form slots (LPFormSection, LPSplitHero, ServicePillar). The /fit
   page no longer renders from here: it runs THE FIT FLOW (one path, 9/11)
   in src/components/fit/FitFlow.tsx on the same verdict rules.
...
export interface FitCheckProps {
  /** Accepted but ignored: every caller is the embed. The 'page' branch
   *  retired 9/11 (the /fit page renders FitFlow); the type stays so
...
export function FitCheck({
  ...
  /* ─── The quiz standing where the open form used to ─── */
  return (
