...
interface ClientLogoBarProps {
  tagline?: string;
  /** Real review count — only rendered when provided (don't invent one). */
  reviewCount?: string;
  rating?: string;
}

export function ClientLogoBar({
  tagline = `Trusted by ${TRACK_RECORD.socialAccountsManagedLong} across Louisiana and Texas`,
  reviewCount,
  rating = TRACK_RECORD.googleRating,
}: ClientLogoBarProps) {
  return (
    <section className="relative py-10 lg:py-14 border-b border-border/10">
      <div className="max-w-[1440px] mx-auto px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, ease: ease as any }}>
          {/* Tagline */}
          <p className="text-center text-xs text-faint font-bold uppercase tracking-[0.2em] mb-6">{tagline}</p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-faint font-medium">
                {reviewCount ? `from ${reviewCount} reviews` : 'on Google'}
              </span>
            </div>
            ...
            <div className="flex items-center gap-2 bg-white/[0.03] border border-border/20 rounded-lg px-3 py-1.5">
              <div className="w-5 h-5 rounded bg-gradient-to-br from-blue-500/30 to-green-500/30 flex items-center justify-center">
                <span className="text-[8px] font-black text-white/80">G</span>
              </div>
              <span className="text-[10px] font-bold text-faint uppercase tracking-wider">Google Partner</span>
            </div>
          </div>
        </motion.div>
      </div>
